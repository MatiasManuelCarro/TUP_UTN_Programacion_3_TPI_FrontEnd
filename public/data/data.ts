...
export async function getCategories(): Promise<ICategory[]> {
  const categorias = await loadJSON<ICategory[]>("/public/data/categorias.json");
  return categorias.filter(c => !c.eliminado);
}

export async function getProducts(): Promise<Product[]> {
  const productos = await loadJSON<Product[]>("/public/data/productos.json");
  return productos.filter(p => !p.eliminado);
}


export async function getOrders(): Promise<IOrder[]> {
  const pedidos = await loadJSON<IOrder[]>("/public/data/pedidos.json");
  return pedidos; 
}

export async function getUsers(): Promise<IUser[]> {
  const usuarios = await loadJSON<IUser[]>("/public/data/usuarios.json");
  return usuarios; 
}
/*
...
export const PRODUCTS: Product[] = [
  ...
  },
  {
    ...
    stock: 60,
    imagen: "empanadas_jamon_queso.jpg",
    disponible: true,
    categorias: [categorias[4]],
  },
  {
  ...
  },
  {
    ...
    stock: 0,
    imagen: "hamburguesa_crispy_chicken.jpg",
    disponible: false,
    categorias: [categorias[1]],
  },
  {
